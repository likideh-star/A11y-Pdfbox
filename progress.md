...
| 2         | Unified Public API and Intermediate Document Model    | Partially Done | Declarative POJOs, shared intermediate model records, and a fluent/declarative conversion layer are implemented with equivalence/default/validation tests for heading, paragraph, list, and figure paths; broader node coverage (tables/TOC/page settings semantics) remains pending. |
...
- Continue Milestone 2 and 6 in parallel: extend intermediate/declarative node coverage (table, TOC, richer page semantics) while hardening multi-column edge cases before pass-2 renderer wiring.
